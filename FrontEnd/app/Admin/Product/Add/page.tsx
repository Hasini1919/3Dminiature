...
const frameColors = ["Black", "Brown", "Red"];
const themeColors = ["Blue", "Pink", "Green"];
              ...
              {themeColors.map((color) => (
                ...
                >
                  <input
                    ...
                    onChange={() => handlethemeColor(color)}
                    className="rounded border-gray-300"
                  />
                  <span>{color}</span>
                </label>
...
const AddProduct = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (formData.frameColor.length === 0) {
      alert("At least one frame color should be selected.");
      return;
    }

    if (formData.themeColor.length === 0) {
      alert("At least one theme color should be selected.");
      return;
    }
